		...
		istio_mixer_adapter_sample_myapa.TemplateName: {
			...
			InferType: func(cp proto.Message, tEvalFn template.TypeEvalFn) (proto.Message, error) {
				...
				BuildTemplate = func(param *istio_mixer_adapter_sample_myapa.InstanceParam,
					...
					if param.OptionalString == "" || param.OptionalString == emptyQuotes {
						return nil, fmt.Errorf("expression for field '%s' cannot be empty", path+"OptionalString")
					}
					if t, e := tEvalFn(param.OptionalString); e != nil || t != istio_mixer_v1_config_descriptor.STRING {
						if e != nil {
							return nil, fmt.Errorf("failed to evaluate expression for field '%s': %v", path+"OptionalString", e)
						}
						return nil, fmt.Errorf("error type checking for field '%s': Evaluated expression type %v want %v", path+"OptionalString", t, istio_mixer_v1_config_descriptor.STRING)
					}
			...
			ProcessGenAttrs: func(ctx context.Context, instName string, inst proto.Message, attrs attribute.Bag,
				...
				BuildTemplate = func(instName string,
					...
					*istio_mixer_adapter_sample_myapa.Instance, error) {
					...
					OptionalString, err := mapper.Eval(param.OptionalString, attrs)

					if err != nil {
						msg := fmt.Sprintf("failed to evaluate field '%s' for instance '%s': %v", path+"OptionalString", instName, err)
						glog.Error(msg)
						return nil, errors.New(msg)
					}
					...
					return &istio_mixer_adapter_sample_myapa.Instance{
						...
						OptionalIP: OptionalIP.([]byte),

						OptionalString: OptionalString.(string),
					}, nil
				}
			...
			ProcessReport: func(ctx context.Context, insts map[string]proto.Message, attrs attribute.Bag, mapper expr.Evaluator, handler adapter.Handler) error {
				...
				BuildRes2 = func(instName string,
					...
					*istio_mixer_adapter_sample_report.Res2, error) {
					...
					IpAddr3, err := mapper.Eval(param.IpAddr, attrs)
					IpAddr2 := IpAddr3.([]uint8)
					var IpAddr interface{}
						IpAddr = net.IP(IpAddr2)
